...
// Potential performance bottleneck 'await getConsulValue('witnesses-service-address')'
...
export const getWitnessById = async (id: string): Promise<{ id: string }> => {
  const result = await (
    await fetch(`${await getConsulValue("witnesses-service-address")}/${id}`, {
      method: "GET",
    })
  ).json();
